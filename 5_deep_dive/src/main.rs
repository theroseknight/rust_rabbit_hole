fn main() {
    ...
    let _x = 5;
    // x = 6;
    // The above code will not compile because by default x is not mutable

    let mut x = 5;
    println!("{}",x);
    x = 60;
    // The above code will compile because we made x mutable binding

    let y = x;
    println!("{}",y);

    // y = 40;
    // The above code will not compile because even though x is mutable, y is not pointing to a mutable reference of x

    let y = &mut x;
    // y = 40;
    // The above code will not compile because ever though y is now bound to a mutable reference of x, you must use *y
    *y = 40;
    println!("{}",y);
    ...
    // Structs in rust are a custom data type that appear to function identically to a Class in Ruby.

    // First you define it by giving it a camelCase name with the first word also captitalized

    // NOTICE: Structs are always placed BEFORE the main function, the code below works but is bad form because the struct would be accesible prior to this point
    // in the program.  It's only included here instead of before main because this section teaches structs.
    struct PointInSpace {
        x: i32,
        y: i32,
    }
